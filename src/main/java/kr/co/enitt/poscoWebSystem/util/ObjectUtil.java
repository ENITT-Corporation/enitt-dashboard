...
public class ObjectUtil {
	...
	public static Long diffSec(String now, String day) throws Exception{
		Date formatNow = new SimpleDateFormat("yyyy-MM-dd").parse(now);
		Date formatDay = new SimpleDateFormat("yyyy-MM-dd").parse(day);
		
		long diffSec = formatNow.getTime() - formatDay.getTime() / 1000;
		
		return diffSec;
	}
	
	public static Long diffMin(String now, String day) throws Exception{
		Date formatNow = new SimpleDateFormat("yyyy-MM-dd").parse(now);
		Date formatDay = new SimpleDateFormat("yyyy-MM-dd").parse(day);
		
		long diffMin = formatNow.getTime() - formatDay.getTime() / 60000;
		
		return diffMin;
	}
	
	
	public static Long diffHour(String now, String day) throws Exception{
		Date formatNow = new SimpleDateFormat("yyyy-MM-dd").parse(now);
		Date formatDay = new SimpleDateFormat("yyyy-MM-dd").parse(day);
		
		long diffHour = formatNow.getTime() - formatDay.getTime() / 3600000;
		
		return diffHour;
	}
	
	
	public static Long diffDays(String now, String day) throws Exception{
		Date formatNow = new SimpleDateFormat("yyyy-MM-dd").parse(now);
		Date formatDay = new SimpleDateFormat("yyyy-MM-dd").parse(day);
		
		long diffDays = ((formatNow.getTime() - formatDay.getTime()) / 1000) / (24*60*60);
